...
- **Connections**: `ConnId = node_id << 48 | local number`. Local numbers come from durably reserved blocks: before handing out `[a, a + 65536)` the node writes `a + 65536` to `data_dir/conn-ids` (temporary file, fsync, rename, directory fsync), and a new process starts at `max(persisted, highest local number in the replicated state + 1)`, so no number is reused even if a process crashes before any of its `Connect`s is committed. (Only a node whose `data_dir` lost that file but whose connections the state has seen, i.e. a wiped node, starts `2^20` above the highest number seen.) The owner (the node holding the socket) forwards inputs to the leader (`ForwardRequest`), one in flight per connection; the state machine drops duplicates by `(conn, seq)`. Each node delivers the replies for its own connections from its own apply, so leader changes lose no replies and keep waiting reserves and reservations of surviving nodes.
...
- **Bootstrap and rejoin**: every initial node is started with `--cluster-init` and the same `[[cluster.peer]]` list (openraft accepts the same membership initialized on each node). `--cluster-init` is refused if `data_dir` holds state or a rejoin marker; before initializing, the node asks every reachable peer with an empty forward whether it belongs to a running cluster (it has applied, or learned the commit of, an entry beyond the bootstrap membership), and if one does, it does not bootstrap but rejoins (with a warning). A node started with an empty `data_dir` without `--cluster-init` (a new disk, or a node that lost its data) *rejoins*: it may have acknowledged entries it no longer has, and voting with that log could elect a leader missing a committed entry. It writes `data_dir/rejoin` durably, disables its elections (`runtime_config().elect(false)`), closes its vote gate and serves no clients; it asks the leader to propose `DropNode(self)` and, once it has applied that entry (an index learned from a leader after it started, so everything committed before is in its log), removes the marker durably, re-enables elections and opens the gate. A crash in between keeps the marker, so the node rejoins again. Limits: a rejoining node needs a leader elected by the other nodes, so at most a minority may rejoin at once; the peer check of `--cluster-init` only protects against peers it can reach.
