...
A 3- or 5-node cluster behaves like one reference server, with the differences below. The differential corpus runs against a 3-node cluster twice, with every client connection on the leader (`ref_vs_cluster_leader`) and on one follower (`ref_vs_cluster_follower`); see `tests/compat/src/cluster.rs`. Real clients (Python, Go) run against a cluster through a follower, including a leader kill while a reservation is held and a reserve is waiting (`SMOKE_CLUSTER=1`, `SMOKE_CLUSTER_KILL=1` in `clients/run-smoke.sh`).

| # | Difference | Reason | Test |
|---|---|---|---|
| C1 | `stats` shows the `pid`, `hostname`, `id`, `rusage-*` and `version` of the node the client is connected to. | Per-process values; the engine state is replicated, the process is not. | masked (already volatile in every mode) |
| C2 | `uptime` counts from the cluster's first log entry, not from the process start. | Replicated time. | masked |
| C3 | `binlog-oldest-index`, `binlog-current-index`, `binlog-records-migrated`, `binlog-records-written` and `file` (stats-job) are always 0; `binlog-max-size` still reports `-s`. | The Raft log replaces the binlog (`-b` with `[cluster]` is a configuration error). | cluster suites mask exactly these five fields |
| C4 | A `-z` above 33,488,896 bytes (one 32 MiB cluster frame minus 64 KiB) is a configuration error at startup. This includes every value the reference clamps to 1 GiB. | A put is one log entry and must fit in one cluster frame. | `max_job_size_clamped`, `max_job_size_negative_wraps` and `max_job_size_overflow_saturates` are excluded from the cluster suites, which check that each still fails with this error |
| C5 | Restarting every node (SIGTERM or kill -9, same data directories) keeps every committed change: jobs in every state, job ids, cumulative counters (`cmd-*`, `total-jobs`, `total-connections`, `job-timeouts`, per-tube counters), tubes that still hold jobs or references, pause and drain state. Connections of the stopped processes are disconnected when their node restarts (in ascending connection order, one `DropNode` entry), so their reservations return to ready and their waiting reserves end, exactly as if each client had disconnected. None of the binlog recovery rules apply (Binlog items 2 to 5: counters reset, pause and drain lost, the `buries` bump, replay order of buried jobs and tubes). | Every engine input is in the Raft log, which survives a full restart; a restart is a disconnect of every client, nothing more. | the cluster suites compare every `restart` / `crash` case (30 cases) against a reference that keeps running and only sees each connection close, in opening order, 20 ms apart (`DisconnectOnRestart`); all pass |
| C6 | On a graceful shutdown (SIGTERM) a node closes its client connections itself, and their disconnects reach the log in the order the connection tasks end, so the order among several connections is not fixed (like simultaneous disconnects on the reference, D11). After kill -9 the order is the connection order (C5). | Connection tasks end concurrently. | not exercised by the corpus: the harness's `restart` closes the case's connections in opening order before the SIGTERM, to keep the order deterministic |
| C7 | SIGUSR1 on any node puts the whole cluster in drain mode, and drain mode survives a full restart (C5). | Drain is a replicated input. | `drain_*.bt`, `binlog_drain_mode_not_persisted.bt` (signal sent to the client's node) |
| C8 | A client whose node is cut off from the majority gets no replies until the node rejoins or `cluster.node_timeout` closes its connection; it never gets a reply for an uncommitted change. Every reply waits for a majority commit (about 1 ms on one machine). | Linearizability. | chaos tests (P3-T6); the corpus, including its timing-sensitive cases (`reserve-with-timeout`, TTR, `DEADLINE_SOON`, pause expiry), passes unchanged against a cluster |
| C9 | When a node's forward queue is full (100,000 inputs or 128 MiB), new connections to that node are closed at accept and a `put` gets `OUT_OF_MEMORY`. While a node is cut off from the leader, rejoining after data loss, or shutting down, new connections are closed at accept without a reply. | Bounded memory and log growth under overload and partitions; a node that cannot commit must not accept work. | `reconnect_storm_to_an_isolated_node` (chaos findings), cluster integration tests |
