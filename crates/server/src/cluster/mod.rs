...
//! # Startup ([`start`])
//!
//! Open the storage (a locked data directory exits with status 10), start
//! Raft with the TCP/TLS network and the cluster listener, bootstrap the
//! membership with `--cluster-init`, wait until a leader is known and this
//! node has applied everything it knows to be committed, then close out
//! the connections of this node's previous process with `DropNode(self)`
//! and wait until that is applied here. Only then are clients accepted,
//! numbered above every local number the replicated state has seen for
//! this node (plus a margin, [`CONN_ID_GAP`]).
...
use bstk_raft::listener::{ClusterListener, ListenerConfig};
use bstk_raft::storage::{self, LogOptions, LogStore, ReplySink, SmOptions, StateHandle};
use bstk_raft::tls::ClusterTls;
use bstk_raft::{CONN_SEQ_BITS, NodeId, Op, Request, TypeConfig, conn_id, owner_of};
...
/// Connection numbers of a new process start this far above the highest
/// number the replicated state has seen for this node, so that a `Connect`
/// the previous process forwarded but that commits only after this restart
/// cannot make this process's connections look old (the state machine
/// accepts a `Connect` only above every earlier one of the same node). It
/// also keeps them above the bound of any `DropNode` proposed for the old
/// process, even one that commits late.
...
pub struct ClusterNode {
    ...
    /// Connection ids of this process start here.
...
pub async fn start(args: StartArgs<'_>) -> Result<ClusterNode, StartError> {
    ...
    let (log, sm) = match storage::open(&c.data_dir, LogOptions::default(), sm_opts) {
        ...
        Err(e) => {
            return Err(StartError::Other(format!(
                "cannot open cluster.data_dir {}: {e}",
                c.data_dir.display()
            )));
        }
    };
    ...
    let core = Arc::new(Core {
        ...
        peers: c.peers.keys().copied().collect(),
        clients,
        status: Status {
    ...
    tracing::info!(node = id, addr = %listener.local_addr(), "cluster listener started");
    ...
    // serves nothing until clients connect, but the readiness monitor and
    // the leader's duties are needed while this node catches up).
    ...
    if raft.is_initialized().await.unwrap_or(false) {
        ...
        tracing::warn!(
            "cluster.data_dir holds no state: waiting to be contacted by the leader \
             (start one node with --cluster-init to bootstrap a new cluster)"
        );
    }
    core.drop_previous_connections().await;

    let first_local = core.state.highest_local(id).saturating_add(CONN_ID_GAP) + 1;
    if first_local >= 1 << CONN_SEQ_BITS {
        for t in tasks.drain(..) {
            t.abort();
        }
        return Err(StartError::Other(
            "connection numbers of this node are exhausted".into(),
        ));
    }
    ...
    Ok(ClusterNode {
        ...
        first_conn_id: conn_id(id, first_local),
...
/// `--cluster-init` refuses a data directory that already holds state.
