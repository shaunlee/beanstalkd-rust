//! Cluster differential mode, clients on a follower: every case in `cases/`
//! runs with the reference as server A and a fresh 3-node `beanstalkd-rs`
//! cluster as server B, every client connection going to one fixed follower. See
...
#[test]
fn ref_vs_cluster_follower_all_cases_pass() {
    ...
    assert_eq!(
        failed,
        0,
        "{failed} of {} ref-vs-cluster (follower) case(s) failed (see report above); {passed} passed",
        passed + failed
    );
}
